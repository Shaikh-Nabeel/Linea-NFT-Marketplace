...
const contractAddress = '0xdaa92ae32234c076675a8bbb52396906d9444f58';
const abi = [
	{
		"inputs": [],
		...
		"outputs": [
			{
				...
				"internalType": "struct EduChainNFTMarketplace.NFT[]",
				"name": "",
				"type": "tuple[]"
			}
		],
...
function App() {
	...
	console.log("addresss", list, error);
	...
	if (error || list == undefined) return <p className='min-h-screen'> <div className='w-full font-extrabold text-center text-2xl text-white mt-10'>
		Welcome to EduChain NFT Marketplace
	</div> </p>;
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-2 mt-2 min-h-screen">
					...
					{list.map((nft) => (
						...
						<div key={nft.tokenId} className="flex justify-center">
							<LoadImage tokenId={nft.tokenId} price={nft.price} owner={nft.owner} isSold={nft.isForSale} abi={abi} contractAddress={contractAddress} />
						</div>
					))}
				</div>
			</div>
